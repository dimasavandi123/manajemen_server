    ...
    <div class="col-12 col-lg-12 col-xxl-9 d-flex">
        <div class="card flex-fill">
            ...
            <div class="col-12 col-lg-8">
                <form action="{{ route('data-vm.store') }}" method="POST">
                    @csrf
                    <div class="form-group mb-3">
                        <label for="" class="form-label">Nama Vm</label>
                        <input type="text" class="form-control" name="nama_vm">
                    </div>
                    <div class="form-group mb-3">
                        <label for="" class="form-label">Nama OS</label>
                        <input type="text" class="form-control" name="os_vm">
                    </div>
                    <div class="form-group mb-3">
                        <label for="" class="form-label">Ip Public Vm</label>
                        <input type="text" class="form-control" name="ip_public_vm">
                    </div>
                    <div class="form-group mb-3">
                        <label for="" class="form-label">Ip Local Vm</label>
                        <input type="text" class="form-control" name="ip_local_vm">
                    </div>
                    <div class="form-group mb-3">
                        ...
                        @foreach ($serverData as $server)
                                <option value="{{ $server->id }}">{{ $server->merk_server}} -- {{ $server->ip_server }}</option>
                        @endforeach
                        </select>
                    </div>
                    <div class="form-group mb-3">
