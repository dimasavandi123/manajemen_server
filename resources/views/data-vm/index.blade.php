    ...
    <div class="col-12 col-lg-12 col-xxl-9 d-flex">
        <div class="card flex-fill">
            <div class="card-header">
                ...
                <a href="{{ route('data-vm.create') }}" class="btn btn-info float-end"><i class="align-middle" data-feather="plus"></i> </a>
            ...
            <div class="table-responsive">
                <table class="table table-hover table-striped my-0 mb-3">
                    ...
                    <tbody>
                      @foreach ($dataVm as $no => $vm)
                          <tr>
                            <input type="hidden" class="delete_id" value="{{ $vm->id }}">
                            <th>{{ $no+1 }}</th>
                            <td>{{ $vm->nama_vm }}</td>
                            <td>{{ $vm->os_vm }}</td> 
                            <td>{{ $vm->ip_public_vm }}</td>
                            <td>{{ $vm->ip_local_vm }}</td>
                         
                            <td>
                      
                                    <form action="{{ route('data-vm.destroy',$vm->id) }}" method="POST">
                                        @method('delete')
                                        @csrf
                                        <div class="btn-group">
                                            <a href="{{ route('data-vm.edit',$vm->id) }}" class="btn btn-primary btn-sm"><i class="align-middle" data-feather="edit"></i></a>
